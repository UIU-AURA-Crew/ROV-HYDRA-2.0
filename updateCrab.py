...
class PrototypeSoftware(QWidget):
    ...
    def keyPressEvent(self, event):
        key = event.key()
        if key == Qt.Key.Key_W:  #this key is for up
            print("Key pressed: Moving Up")
            self.send_command('b')
            self.joystick_up.setStyleSheet("background-color: yellow")
            QTimer.singleShot(100, lambda: self.joystick_up.setStyleSheet(""))
        elif key == Qt.Key.Key_S:  #this key is for down
            print("Key pressed: Moving Down")
            self.send_command('a')
            self.joystick_down.setStyleSheet("background-color: yellow")
            QTimer.singleShot(100, lambda: self.joystick_down.setStyleSheet(""))
        elif key == Qt.Key.Key_A:  # this key is for left
            print("Key pressed: Moving Left")
            self.send_command('c')
            self.joystick_left.setStyleSheet("background-color: yellow")
            QTimer.singleShot(100, lambda: self.joystick_left.setStyleSheet(""))
        elif key == Qt.Key.Key_D:  # this key is for Right
            print("Key pressed: Moving Right")
            self.send_command('d')
            self.joystick_right.setStyleSheet("background-color: yellow")
            QTimer.singleShot(100, lambda: self.joystick_right.setStyleSheet(""))
        elif key == Qt.Key.Key_X:  # this key is for Grip Open
            print("Key pressed: Grip Open")
            self.send_command('x')
            self.grip_button.setStyleSheet("background-color: yellow")
            QTimer.singleShot(100, lambda: self.grip_button.setStyleSheet(""))
        elif key == Qt.Key.Key_Y:  # this key is for Grip Close
            print("Key pressed: Grip Close")
            self.send_command('y')
            self.grip_button.setStyleSheet("background-color: yellow")
            QTimer.singleShot(100, lambda: self.grip_button.setStyleSheet(""))
        elif key == Qt.Key.Key_Q:  # this key is for Quit Command
            print("Key pressed: Quit Command")
            self.send_command('q')
            self.grip_button.setStyleSheet("background-color: yellow")
            QTimer.singleShot(100, lambda: self.grip_button.setStyleSheet(""))
